...
==================
package student
func WordAnatomy(word string, pfx, sfx []string) string {
var p, s string
for _, pr := range pfx {
if len(word) >= len(pr) && word[:len(pr)] == pr {
p = pr
break
}
}
for _, sf := range sfx {
if len(word) >= len(sf) && word[len(word)-len(sf):] == sf {
s = sf
break
}
}
